...
func (mgr *Mgr) Merge(lhs *map[string]*listener.PipeDefinition) {
	...
	log.Println("LOnly", LOnly)
	log.Println("Common", Common)
	log.Println("ROnly", ROnly)
	// Not Common or in the right hand (new kubeConfig) set, are now
	// vestiges of the prior (lhs) set
	for _, k := range LOnly {
		{
			defer trace.Tracer.Detailed(trace.Detail).Enable(trace.Enabled).ScopedTrace("range LOnly")()
	...
	for _, k := range Common {
		{
			defer trace.Tracer.Detailed(trace.Detail).Enable(trace.Enabled).ScopedTrace("range Common")()
	...
	for _, k := range ROnly {
		defer trace.Tracer.Detailed(trace.Detail).Enable(trace.Enabled).ScopedTrace("range ROnly")()
		log.Println("right only rhs[k]", k, (*rhs)[k])
		(*lhs)[k] = listener.NewPipeDefinition((*rhs)[k])
		mgr.Listeners[k] = NewManagedListener((*rhs)[k], kubeConfig)
		mgr.Listeners[k].Open()
		// log.Println("lhs[k]", k, (*lhs)[k])
		// log.Println("rhs[k]", k, (*rhs)[k])
	}
	mgr.LoadEndpoints()
}
	...
	go func() {
		for {
			...
			if err != nil {
				...
				// log.Println(len(watcher.Events))
				select {
				case event := <-watcher.Events:
					...
					if err == nil {
						// defer trace.Tracer.Detailed(trace.Detail).Enable(trace.Enabled).ScopedTrace(msg)()
...
// Connector types like pipeDefs and links
